...
| **Vendedor / Comercial** | Pessoa (pode ser o próprio dono do negócio) que atende leads de contato direto **e** realiza a prospecção ativa (busca local), conduzindo ambos até o fechamento | Ver todos os leads organizados num único funil, saber quem priorizar, registrar buscas e não deixar ninguém esquecido |
...
- **Meta e Telegram**: ainda não há integração automatizada (webhook/API) com esses canais no MVP, então **o registro do lead é sempre feito pelo próprio usuário (vendedor)** — ele atende o cliente no canal de origem e, em seguida, cadastra manualmente o lead no sistema, informando o canal de origem, a mensagem/contexto e os dados de qualificação.
...
flowchart LR
    ...
    D --> F[Vendedor apresenta produto<br/>pronto ou personalizado]
    F --> G[Funil: Contatado → Proposta → Negociação]
    G --> H{Resultado}
...
flowchart LR
    ...
    C --> D[Vendedor entra em contato<br/>oferecendo produto pronto ou personalizado]
...
- Integração automatizada (webhook/API) com os canais de contato direto, reduzindo a necessidade de cadastro manual pelo vendedor.
...
- **Interação**: histórico de contatos com o lead (mensagens trocadas, ligações, observações do vendedor), com data/hora e autor.
...
- Um **Lead** possui várias **Interações** ao longo do tempo.
...
- **Cadastro manual de leads via Meta/Telegram**: como não há integração automática com esses canais no MVP, a qualidade dos dados depende do vendedor registrar o lead de forma completa e tempestiva logo após o atendimento — atrasos ou registros incompletos prejudicam a qualificação e as métricas.
...
6. Confirmar acesso às APIs necessárias para a busca local (Google Maps/Places e portal(is) de CNPJ/Receita Federal ou provedor de dados públicos de CNPJ) e às APIs de notificação (WhatsApp, Telegram, e-mail transacional).
7. Planejar a arquitetura do backend em Java (ex.: Spring Boot), o modelo de dados inicial (incluindo usuários/permissões e preferências de notificação) e definir o setup de testes (framework, cobertura mínima) para conduzir o desenvolvimento em **TDD** desde o primeiro módulo.
8. Planejar um MVP funcional em poucas semanas, focado nas funcionalidades essenciais (permissões, endpoint de API para leads do site, cadastro manual de leads via Meta/Telegram, busca local automatizada, qualificação básica, funil e notificações configuráveis), deixando integrações avançadas (webhooks de Meta/Telegram, novas redes sociais) para uma segunda fase.
